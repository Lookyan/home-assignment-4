...
class WriteLetterTest2(unittest.TestCase):
    ...
    def test_template_save_load(self):
        ...
        # text2 = 'hohoho'
        self.content_edit.change_text(text)
        # self.content_edit.add_text(text1)
        # self.content_edit.add_text(text2)
        # text = text0 + text1 + text2
        main_toolbar = self.compose_page.main_toolbar()
        main_toolbar.save_template()
        self.assertTrue(main_toolbar.check_template(text))
        # main_toolbar.cancel()

        compose_page = ComposePage(self.driver)
        compose_page.open()
        main_toolbar = compose_page.main_toolbar()
        main_toolbar.use_template(text)
        self.assertEqual(compose_page.content_edit().get_text(), text)
        main_toolbar.cancel()

        template_page = TemplatesPage(self.driver)
        template_page.open()
        template_page.letter_list().delete_letter_with_text(text)
    ...
    def test_send_letter(self):
        ...
        # b = self.driver.find_element_by_xpath("//div[contains(@class, 'message-sent') and a]")
        # a =5